...
def find_most_similar_sentences(question, lecture, n):
    ...
    cos_similarities = [
        cos_sim(q_embedding, sentence) for sentence in lecture.embeddings
    ]
    top_entries = sorted(enumerate(cos_similarities), key=lambda x: x[1], reverse=True)[
        :n
    ]
    ...
    return tuple(
        zip(
            [entry[1] for entry in top_entries],
            [lecture.content[entry[0]] for entry in top_entries],
        )
    )
